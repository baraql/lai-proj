...
class IterableParquetDataset(IterableDataset):
    ...
    def __next__(self):
        ...
        while len(self.token_buffer) < self.sequence_length +1 :
            sample_str = str(self.parquet_ds["text"][i % self.real_length])
            curr = self.tokenizer.encode_plus(sample_str,
                                        #max_length=self.sequence_length + 1 - len(input_ids),
                                        padding=False,
                                        truncation=False,
                                        )
            self.token_buffer += curr['input_ids']
            self.token_buffer.append(self.bos_token_id)
            i+=1
        
        input_curr = self.token_buffer[:self.sequence_length +1]
        
        input_ids = torch.LongTensor(input_curr).unsqueeze(0) # (1, sequence_length+1)
        inputs = input_ids[:, :-1].clone()
        labels = input_ids[:, 1:]
